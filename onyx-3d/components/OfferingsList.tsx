...
/** Editorial row list of offerings — numbered, divided by hairlines, each linking to its detail on /angebot. */
        ...
        <div style={{ borderTop: "1px solid var(--hairline)" }}>
          {offerings.map((o, i) => (
            <Link
              key={o.slug}
              href={`/angebot#${o.slug}`}
              className="offering-row flex items-center justify-between gap-6 py-7"
              style={{ borderBottom: "1px solid var(--hairline)" }}
            >
              <div className="flex items-baseline gap-6 min-w-0">
                <span className="mono" style={{ fontSize: 14, color: "var(--warm-grey-faint)", flexShrink: 0 }}>
                  {String(i + 1).padStart(2, "0")}
                </span>
                <div className="min-w-0">
                  <div style={{ fontWeight: 700, fontSize: "clamp(1.1rem, 2vw, 1.35rem)" }}>{o.title}</div>
                  <div style={{ color: "var(--warm-grey-dim)", fontSize: "0.95rem", marginTop: 4 }}>{o.subtitle}</div>
                </div>
              </div>
